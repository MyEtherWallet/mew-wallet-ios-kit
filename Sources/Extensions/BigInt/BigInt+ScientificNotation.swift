//
//  BigInt+ScientificNotation.swift
//  MEWwalletKit
//
...
extension BigInt {
  init?(scienceNotation value: String) {
    ...
    case ...1:
      guard let val = BigInt(value, radix: 10) else { return nil }
      self = val
    case 2:
      ...
      var a = BigInt(pow(10, power))
      switch splittedNumber.count {
      case 1:
        guard let number = BigInt(splittedNumber[0], radix: 10) else { return nil }
        self = number * a
      case 2:
        let stringNumber = String(splittedNumber[0]) + String(splittedNumber[1])
        let am = BigInt(pow(10, Double(splittedNumber[1].count)))
        a = a / am
        guard let number = BigInt(stringNumber, radix: 10) else { return nil }
        self = number * a
      default:
        return nil
      }
